...
export type RollbackStrategyDeployStep45Props = {
  setLoading: any;
...
const RollbackStrategyDeployStep45: React.FC<RollbackStrategyDeployStep45Props> = (props) => {
  const {
    setLoading,
  ...
  const nextDis =
    ...
    currentAppDeployData?.status === 608
      ? false
      : true;
  ...
  return (
    <Card
      ...
      extra={
        <Space size="large">
          {currentAppDeployData?.originalStage === 1101 &&
            currentAppDeployData.status !== 608 &&
            !isBuild && (
    ...
    >
      <ProForm
        ...
        submitter={{
          render: (props) => {
            if (!_.isEmpty(currentAppDeployData) && currentAppDeployData?.originalStage === 1101) {
              return (
              ...
              {currentAppDeployData?.originalStage === 1101 && (
                <>
                  <Button
                    ...
                    disabled={currentAppDeployData.status === 608 || buildIsStart}
                  >
                    开始回滚
                  </Button>
                </>
...
export default RollbackStrategyDeployStep45;
