...
const RollbackStrategyDeployStep40: React.FC<RollbackStrategyDeployStep40Props> = (props) => {
  const {
    setLoading,
  ...
  const nextDis =
    ...
    currentAppDeployData?.status === 602
      ? false
      : true;
  ...
  return (
    <Card
      ...
      extra={
        <Space size="large">
          {currentAppDeployData?.originalStage === 1100 &&
            currentAppDeployData.status !== 602 &&
            !isBuild && (
    ...
    >
      <ProForm
        ...
        submitter={{
          render: (props) => {
            if (!_.isEmpty(currentAppDeployData) && currentAppDeployData?.originalStage === 1100) {
              return (
              ...
              {currentAppDeployData?.originalStage === 1100 && (
                <>
                  <Button
                    ...
                    disabled={currentAppDeployData.status === 602 || buildIsStart}
                  >
                    开始回滚
                  </Button>
                </>
...
export default RollbackStrategyDeployStep40;
