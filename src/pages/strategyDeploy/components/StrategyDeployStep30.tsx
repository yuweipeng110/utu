...
export type StrategyDeployStep30Props = {
  setLoading: any;
  setLoadingTip?: any;
...
// 策略审批
const StrategyDeployStep30: React.FC<StrategyDeployStep30Props> = (props) => {
  const {
    setLoading,
    setLoadingTip,
  ...
  const queryAndUpdateApprovelFlowRequest = async () => {
    setLoading(true);
    setLoadingTip('创建/查询/处理审批流时间较长，请等待...');
  ...
  const approveStatusRender = (status: number) => {
    ...
    return (
      <span style={{ color: colorVal, fontWeight: 'bold' }}>
        {StrategyDeployApproveStatus[status]}
      </span>
    );
  };
