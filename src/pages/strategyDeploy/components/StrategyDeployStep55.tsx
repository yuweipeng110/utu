...
const StrategyDeployStep55: React.FC<StrategyDeployStep50Props> = (props) => {
  const {
    setLoading,
  ...
  const [packDeployData, setPackDeployData] = useState(Object.create(null));
  ...
  const packDeployRequest = async () => {
    const res = await packDeployDetail({
      ...
      appId: Number(appId),
      deployRecordList: _.isEmpty(packDeployData)
        ? null
        : !_.isEmpty(packDeployData?.firstMessage)
        ? packDeployData?.firstMessage
        : packDeployData?.externalMessage,
    });
  ...
  const nextDis =
    ...
    currentAppDeployData?.status === 367
      ? false
      : true;
  ...
  return (
    <ProCard
      ...
      extra={
        <Space size="large">
          {currentAppDeployData?.originalStage === 701 &&
            currentAppDeployData.status !== 367 &&
            !isBuild && (
    ...
    >
      <ProForm
        ...
        submitter={{
          render: (props) => {
            if (!_.isEmpty(currentAppDeployData) && currentAppDeployData?.originalStage === 701) {
              return (
              ...
              {currentAppDeployData?.originalStage === 701 && (
                <>
                  <Button
                    ...
                    disabled={currentAppDeployData.status === 367 || buildIsStart}
                  >
                    部署线上
                  </Button>
                </>
...
export default StrategyDeployStep55;
