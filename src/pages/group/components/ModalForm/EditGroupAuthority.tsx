...
import { editRoleGroup } from '@/services/group';

export type EditGroupAuthorityProps = {
  actionRef: any;
  visible: boolean;
...
const EditGroupAuthority: React.FC<EditGroupAuthorityProps> = (props) => {
  const { actionRef, currentData, visible, onVisibleChange } = props;
  const [form] = Form.useForm();
  const initialValues = !_.isEmpty(currentData)
    ? {
      ...currentData,
      empUsers: !_.isEmpty(currentData.userList) ?  currentData.userList.map((item) => `${item.empId}-${item.empName}`) : [],
    }
    : {};
  ...
  const { loading, run, cancel } = useRequest(queryBucUserList, {
    ...
    manual: true,
    formatResult: (res) => {
      const uniqData = _.uniqBy(_.concat(currentData.userList || [], res.data), 'empId');
      console.log('uniqData',uniqData);
      
      const options: any = uniqData.map((item) => {
        return {
  ...
  const onSubmit = async (values: any) => {
    ...
    const params = {
      ...values,
      empUsers: values.empUsers.map((item: any) => {
    ...
    const res = await editRoleGroup(params);
    if (res.code !== 1) {
...
export default EditGroupAuthority;
